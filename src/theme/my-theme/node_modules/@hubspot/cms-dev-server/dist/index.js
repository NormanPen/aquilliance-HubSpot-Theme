  ...
  return {
    ...
    load(id) {
      if (id.endsWith("?island") || id.endsWith("?client")) {
        ...
        const modulePath = path.relative(viteRoot, idWithNoParam);
...
function getSubdirectories(dirPath) {
  try {
    return fs5.readdirSync(dirPath, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name);
  } catch {
    return [];
  }
}
function hsmetaCandidatePaths(srcPath) {
  ...
  return subdirs.flatMap(
    (subdir) => HSMETA_FILENAMES.map((filename) => path14.join(srcPath, subdir, filename))
  );
}
function tryParseUid(hsmetaPath) {
  try {
    const hsmeta = JSON.parse(fs5.readFileSync(hsmetaPath, "utf8"));
...
function findHsmetaUid(projectDir, srcDir) {
  const srcPath = path14.join(projectDir, srcDir);
...
async function createDevServer(webRoot, storybook, hubspotCliConfigPathOption, hubspotCliAccountNameOption, sslEnabledOption, fieldGenEnabledOption, themePreviewOptions, fieldBuilderEnabled) {
  ...
  const cmsAssetConfig = getCMSAssetConfigIfExists(webRoot);
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  const absoluteWebRoot = normalizePath10(path17.resolve(process.cwd(), webRoot));
    ...
    const storybookProcess = spawn2(
      ...
      ],
      {
        ...
        env: {
          ...
          hsEnv: authedEnv,
          hublet: authedHublet,
          WEB_ROOT: path17.join(webRoot, "components")
        },
        stdio: "inherit"
      }
    );
