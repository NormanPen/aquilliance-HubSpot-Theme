...
// src/lib/hsmetaUtils.ts
import path from "path";
import fs from "fs";
var HSMETA_FILENAMES = [
  "theme-hsmeta.json",
...
function hsmetaCandidatePaths(srcPath) {
  const subdirs = getSubdirectories(srcPath);
  return subdirs.flatMap(
    (subdir) => HSMETA_FILENAMES.map((filename) => path.join(srcPath, subdir, filename))
  );
}
function tryParseUid(hsmetaPath) {
  try {
    const hsmeta = JSON.parse(fs.readFileSync(hsmetaPath, "utf8"));
    return hsmeta.uid ?? null;
  } catch (parseErr) {
    console.warn(`Could not parse ${hsmetaPath}:`, parseErr.message);
    return null;
  }
}
function findHsmetaUid(projectDir, srcDir) {
  const srcPath = path.join(projectDir, srcDir);
  for (const candidatePath of hsmetaCandidatePaths(srcPath)) {
    if (!fs.existsSync(candidatePath))
      continue;
    const uid = tryParseUid(candidatePath);
    if (uid)
      return uid;
  }
  return null;
  ...
  return promptUser([
    {
      ...
      choices: themeComponents.map((t2) => {
        const themeName = path2.basename(t2.path);
        return {
          name: themeName,
          value: t2.path
        };
...
var getProjectConfig = async (_dir, isUsingLocalWorkspace = false) => {
  ...
  try {
    const config = fs2.readFileSync(configPath, "utf8");
    const projectConfig = JSON.parse(config);
    return {
      projectDir: path2.dirname(configPath),
      projectConfig
    };
  } catch (e) {
...
function loadConfigFile(configPath) {
  if (configPath) {
    try {
      const source = fs2.readFileSync(configPath);
...
async function findProjectComponents(projectSourceDir) {
  ...
  projectFiles.forEach((projectFile) => {
    const { base, dir } = path2.parse(projectFile);
  ...
  if (cards) {
    cards.forEach(({ file }) => {
      if (typeof file === "string") {
        const cardConfigPath = path2.join(appPath, file);
...
function findAndReadHsmeta(projectDir, srcDir = "src") {
  const srcPath = path2.join(projectDir, srcDir);
  const candidates = hsmetaCandidatePaths(srcPath);
  const hsmetaPath = candidates.find((p) => fs2.existsSync(p));
  if (!hsmetaPath) {
    console.error(
      `Error: No hsmeta file (theme-hsmeta.json or cms-assets-hsmeta.json) found in any subdirectory of ${srcPath}. For platformVersion 2025.2 and higher, the project structure requires an hsmeta file.`
    );
    process2.exit(1);
  }
  const hsmetaFilename = path2.basename(hsmetaPath);
  let hsmeta;
  try {
    hsmeta = JSON.parse(fs2.readFileSync(hsmetaPath, "utf8"));
  } catch (e) {
    console.error(
      `Error: Failed to read or parse ${hsmetaFilename} at ${hsmetaPath}`
    );
  ...
  if (!hsmeta.uid) {
    console.error(
      `Error: ${hsmetaFilename} at ${hsmetaPath} is missing required 'uid' field.`
    );
    process2.exit(1);
  }
  return hsmeta;
...
async function determineSrcAndDest(webRoot2, isUsingLocalWorkspace = false) {
  ...
  if (projectDir && projectConfig) {
    ...
    const { dir: themeDir } = path2.parse(themeJsonPath);
    absoluteSrc = themeDir;
    const { base: themeName } = path2.parse(themeDir);
    ...
    if (isNewStructure) {
      ...
      const hsmeta = findAndReadHsmeta(projectDir, srcDir);
...
async function fieldsJsPrompt(filePath, projectDir, skipFiles = []) {
  ...
  const fileChoices = fs2.readdirSync(dirName).filter((file) => FIELDS_FILES.includes(file)).map((file) => path2.resolve(dirName, file)).filter((file) => !skipFiles.includes(file));
  ...
  const fileDir = path2.dirname(fileChoices[0]).replace(projectDirRegex, "");
  const selection = [];
...
function packagePath(metaUrl, projectPath) {
  if (true) {
    return normalizePath(
      path3.resolve(path3.dirname(url.fileURLToPath(metaUrl)), projectPath)
    );
  }
  const parentDirPath = normalizePath(path3.dirname(url.fileURLToPath(metaUrl)));
  const subPath = parentDirPath.replace(/^.+cms-dev-server\/src\//, "");
  const subPathParentDirs = subPath.split("/").map(() => "..");
  return path3.resolve(
    path3.dirname(url.fileURLToPath(metaUrl)),
    path3.join(...subPathParentDirs, projectPath)
  );
}

// ../cms-js-internal/dist/types-BEz622C8.js
import path4 from "path";
import { createHash } from "crypto";
import fs3, { readFileSync, existsSync as existsSync2, readdirSync, lstatSync } from "fs";
  ...
  return {
    ...
    load(id) {
      if (id.endsWith("?island") || id.endsWith("?client")) {
        ...
        const componentExtension = path4.extname(idWithNoParam);
        const componentFilenameWithNoExtension = path4.basename(
          idWithNoParam,
          componentExtension
        );
        const modulePath = path4.relative(viteRoot, idWithNoParam);
...
var isStorybookStory = (componentPath) => {
  return !!path4.basename(componentPath, path4.extname(componentPath)).endsWith(".stories");
};
...
function getCMSAssetConfig(webRoot2) {
  const cmsAssetsPath = path4.join(webRoot2, "cms-assets.json");
  const cmsThemePath = path4.join(webRoot2, "theme.json");
  const configPathToUse = fs3.existsSync(cmsAssetsPath) ? cmsAssetsPath : cmsThemePath;
  const parsedConfig = JSON.parse(
    fs3.readFileSync(configPathToUse).toString()
  );
  return parsedConfig;
}
...
var ENABLED_FEATURES_GATE_MAP = {
  ...
  "PortalsAccessIdentity:DesignManager:UrlSigning": "PortalsAccessIdentity:DesignManager:UrlSigning"
};
...
function toRelativePath(config, path19) {
  return path19.replace(`${config.root}/`, "");
}
...
function analyzeSourceCodeUsages(ctx, moduleInfo, config, code) {
  ...
  traverse(sourceAst, {
    ImportDeclaration(path19) {
      if (t.isStringLiteral(path19.node.source, {
        value: "@hubspot/cms-components"
      })) {
        for (const specifier of path19.node.specifiers) {
    ...
    JSXOpeningElement(path19) {
      if (t.isJSXIdentifier(path19.node.name, {
        name: moduleComponentLocalImportName
      })) {
        let cmsModuleName;
        let cmsModulePath;
        let cmsModuleLabel;
        for (const prop of path19.node.attributes) {
  ...
  while (offsetSoFar < body.length && offsetSoFar !== -1) {
    ...
    if (startOffset >= 0) {
      ...
      if (pathEndOffset >= 0 && pathEndOffset < nextNewlineOffset) {
        const path19 = body.substring(
          startOffset + localJsRenderStartString.length,
          pathEndOffset
        );
        offsetSoFar = pathEndOffset + 1;
        if (startOffset >= 0 && path19) {
...
var getHSProjectConfig = (hsProjectConfigPath) => {
  try {
    return JSON.parse(fs4.readFileSync(hsProjectConfigPath, "utf8"));
  } catch (e) {
    console.warn(
...
var renderLocalError_default = (path19, error) => {
  const stackWithColorClasses = Converter.toHtml(error.stack);
  return `<div data-js-rendered="error" style="${darkErrorStyles}">Error rendering ${path19}<br/><br/><code><pre>${stackWithColorClasses}</pre></code></div>`;
};
...
async function initialPreviewBufferUpload(accountId, src, dest, filePaths, uploadOptions) {
  ...
  const results = await uploadFolder(
    ...
    rest,
    filePaths.map(
      (filePath) => filePath.replaceAll(path5.sep, path5.posix.sep)
    )
  );
  onFinishCallback(results);
}
...
function buildUploadFileToPreviewBufferCallback(accountId, src, dest) {
  return (filePath) => {
    ...
    if (path5.dirname(filePath) === src && ["theme.json", "fields.json"].includes(path5.basename(filePath))) {
...
function getDesignManagerPath(src, dest, file) {
  ...
  return convertToUnixPath(path5.join(dest, relativePath));
}
...
function isModuleFolderChild(filePath) {
  ...
  if (pathParts.length <= 1)
    return false;
  return pathParts.slice(0, pathParts.length - 1).some((part) => getExt(path5.normalize(part)) === MODULE_EXTENSION);
}
...
async function initiatePreviewSession(authedPortalId, absoluteWebRoot, themePreviewOptions2) {
  ...
  if (token) {
    ...
    const nonJSRFilePaths = filePaths.filter(
      (filePath) => !filePath.startsWith(path6.join(absoluteWebRoot, "components"))
    );
    if (tokenStatus === "NEW") {
      ...
      const filesToUpload = [...changedFiles, ...addedFiles].map((filePath) => {
        const relPath = path6.relative(absoluteDest, filePath.substring(1));
        return path6.join(absoluteWebRoot, relPath);
      });
...
var renderComponentForPlaceholder = (indexForThisRender, localRenderPlaceholder, webRoot2, viteServer, pageUrl, basePath, environmentConfig, hsProjectJsonProjectName, webRootProjectComponent) => {
  ...
  const resolvedPath = normalizePath3(
    path7.resolve(webRoot2, partialPath, componentFileName)
  );
...
var URL_SIGNING_GATE = "PortalsAccessIdentity:DesignManager:UrlSigning";
async function signGetUrl(authedPortalId, unsignedUrl) {
  const urlWithPortalId = new URL(unsignedUrl);
  urlWithPortalId.searchParams.set("portalId", `${authedPortalId}`);
  const response = await hsCliHttp7.post(authedPortalId, {
    url: "/url-signer-rest/v1/signing",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json"
    },
    data: JSON.stringify({
      unsignedUrl: urlWithPortalId.toString(),
      httpMethod: "GET"
    })
  });
  return response.data.signedUrl;
}
async function signedGet(authedPortalId, unsignedUrl, headers, gates) {
  if (!isUngatedFor(gates, URL_SIGNING_GATE)) {
    const { data } = await hsCliHttp7.get(authedPortalId, {
      url: unsignedUrl,
      headers
    });
    return data;
  }
  const signedUrl = await signGetUrl(authedPortalId, unsignedUrl);
  const authHeaders = {};
  ...
  if (!response.ok) {
    throw new Error(
      `Preview GET failed: ${response.status} ${response.statusText}`
    );
  }
  return await response.text();
}
var { JSDOM } = jsdom;
async function renderRemoteModulePreview(modulePath, authedPortalId, authedEnv, cookieValue, gates) {
  ...
  const cosRenderingUrl = `https://api.hubspot${authedEnv === "qa" ? "qa" : ""}.com/api/cos-rendering/v1/public/_hcms/preview/template/multi?${queryParams}`;
  const previousHubApiDomainOverride = process.env.HUBAPI_DOMAIN_OVERRIDE;
  const needsDomainOverride = !isUngatedFor(gates, URL_SIGNING_GATE);
  if (needsDomainOverride) {
    process.env.HUBAPI_DOMAIN_OVERRIDE = `api.hubspot`;
  }
  let modulePreviewHtml;
  try {
    modulePreviewHtml = await signedGet(
      authedPortalId,
      cosRenderingUrl,
      {
    ...
    if (needsDomainOverride) {
      if (previousHubApiDomainOverride) {
        process.env.HUBAPI_DOMAIN_OVERRIDE = previousHubApiDomainOverride;
      } else {
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot2, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue, fieldOverrides = {}) {
  const modulePath = normalizePath4(
    path8.resolve(webRoot2, "components/modules/", moduleName)
  );
...
var partialRouteHandler = async (request, response) => {
  ...
  const partialPath = normalizePath5(
    path9.resolve(webRoot2, "components/partials/", partialName)
  );
      ...
      for (const localProjectModule of localProjectModules) {
        ...
        localProjectModulePreviewPromises.push(
          renderLocalProjectModulePreview(
            path9.basename(localProjectModule.path),
            authedPortalId,
            authedEnv,
            authedHublet,
            webRoot2,
...
async function fetchModuleByPath(accountId, path19) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/modules/by-path/${path19}?portalId=${accountId}`
  })).data;
}
async function fetchTemplateByPath(accountId, path19) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/templates/by-path/${path19}?portalId=${accountId}`
  })).data;
}
async function fetchModulePageDependencies(accountId, path19) {
  const url3 = `${DESIGN_MANAGER_API_PATH}/modules/list-pages?modulePath=${path19}`;
...
async function modulePreviewRouteHandler(request, response) {
  ...
  const modulePath = normalizePath8(
    path12.resolve(webRoot2, "components/modules/", moduleName)
  );
...
var hublModuleRouteHandler = async (request, response) => {
  ...
  const { authedPortalId, localPreviewToken, projectName, jsRenderingGates } = request.app.locals;
  const { modulePath } = request.params;
  if (!modulePath) {
    response.status(200).set({ "Content-Type": "text/html" }).end(buildModuleIndex());
    return;
  }
  const projectPrefix = projectName ? "@projects/" : "";
  const calculatedPath = `@preview/${localPreviewToken}/${projectPrefix}${modulePath}.module`;
  let customWidgetInfo;
  try {
    customWidgetInfo = await fetchModuleByPath(authedPortalId, calculatedPath);
  } catch (err) {
    console.error(`Failed to fetch module data for ${calculatedPath}: ${err}`);
    response.status(500).set({ "Content-Type": "text/html" }).end(buildErrorIndex(err));
    return;
  }
  if (!customWidgetInfo || !("moduleId" in customWidgetInfo)) {
    response.status(500).set({ "Content-Type": "text/html" }).end(buildErrorIndex(null));
    return;
  }
  const params = {
  ...
  const previewUrl = getPreviewUrl(request, params);
  try {
    const result = await signedGet(authedPortalId, previewUrl, {}, jsRenderingGates);
...
async function handleGetModulePages(req, res, next) {
  const { path: path19, portalId } = req.query;
  if (!path19 || !portalId) {
...
function handleOpenFile(req, res, next) {
  ...
  try {
    ...
    spawn(command, [path19], { detached: true }).unref();
    res.sendStatus(200);
  } catch (err) {
    next(err);
  }
...
async function fetchProjectByName(accountId, projectName) {
  return (await hsCliHttp9.get(accountId, {
  ...
  return Promise.all(
    modules.moduleNames.map(async (name, index) => {
      const modulePath = normalizePath9(
        path13.resolve(themeWebRoot, "components/modules/", name)
      );
...
function detectPackageManager(projectRoot) {
  if (fs5.existsSync(path13.join(projectRoot, "yarn.lock")))
    return "yarn";
  if (fs5.existsSync(path13.join(projectRoot, "pnpm-lock.yaml")))
    return "pnpm";
  return "npm";
}
...
var appServerRouteHandler = async (req, res, __next) => {
  try {
    ...
    const templatePath = path14.join(appWebRoot, "index.html");
    const serverEntryPath = isProd ? path14.join(appWebRoot, "server/serverEntry.js") : path14.join(appWebRoot, "serverEntry.tsx");
    let render;
    let template = await fs6.readFile(path14.join(templatePath), "utf-8");
    if (isProd) {
...
var createDevServerRoutes = async (viteServer, appLocals, storybookPort) => {
  ...
  if (isProd2) {
    const distAssetsPath = path15.join(appWebRoot, "assets");
...
async function fetchDomains(portalId) {
  try {
    const result = (await hsCliHttp10.get(portalId, {
      url: `/cms/v3/domains`,
      params: { portalId },
      responseType: "json"
    })).data;
...
function findHSProjectJson(startPath) {
  ...
  let dirLevel = 0;
  const maxDirLevel = 4;
  while (path16.dirname(currentDir) !== currentDir && dirLevel < maxDirLevel) {
    const filePath = path16.join(currentDir, HSPROJECT_JSON);
...
var isInModulesFolder = (filePath, projectRoot) => {
  const getEntryPathFromId = (id) => {
    return id.startsWith(projectRoot) ? id.replace(`${projectRoot}${path17.sep}`, "") : "";
  };
  const currentModuleId = path17.normalize(filePath);
  const moduleEntryPath = getEntryPathFromId(currentModuleId);
  const subFolderTokens = moduleEntryPath.split(path17.sep);
  return {
...
var fileContainsAutoGenComment = async (filePath) => {
  if (!fs8.existsSync(filePath))
    return true;
  const stream2 = fs8.createReadStream(filePath);
...
var getIndexFile = (filePath, projectRoot) => {
  ...
  return indexFileName ? { indexFilePath: path17.join(dir, indexFileName), isRootModuleFile } : void 0;
};
...
var maybeUpdateFields = async (filePath, viteServer, projectRoot) => {
  ...
  const indexFileName = path17.parse(indexFilePath).name;
  ...
  if (fields) {
    ...
    const typeFilePath = path17.join(path17.dirname(filePath), typeFileName);
    ...
    if (!hasAutoGenComment)
      return;
    const moduleFolderName = path17.basename(path17.dirname(filePath));
...
var startFieldsTypeGenWatcher = (viteServer, projectRoot) => {
  ...
  moduleSubpaths.forEach((modulePath) => {
    const resolvedPath = path17.resolve(projectRoot, modulePath);
...
function getWebRootProjectComponentFolder(absoluteWebRoot, projectConfigPath) {
  ...
  try {
    const projectDir = path18.dirname(projectConfigPath);
    const configContent = fs9.readFileSync(projectConfigPath, "utf8");
    const projectConfig = JSON.parse(configContent);
    if (hasNewDirectoryStructure(projectConfig.platformVersion)) {
      const srcDir = projectConfig.srcDir || "src";
      const uid = findHsmetaUid(projectDir, srcDir);
      return uid ?? projectConfig.name;
    }
  } catch (e) {
    console.warn(
...
async function createDevServer(webRoot2, storybook2, hubspotCliConfigPathOption2, hubspotCliAccountNameOption2, sslEnabledOption2, fieldGenEnabledOption2, themePreviewOptions2, fieldBuilderEnabled2) {
  ...
  const cmsAssetConfig = getCMSAssetConfigIfExists(webRoot2);
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  let authedHublet;
  try {
    const accountInformation = (await hsCliHttp12.get(authedPortalId, {
  ...
  const absoluteWebRoot = normalizePath10(path18.resolve(process.cwd(), webRoot2));
  ...
  if (fieldGenEnabledOption2) {
    const normalizedAbsoluteWebRoot = path18.normalize(absoluteWebRoot);
  ...
  const __dirname = path18.dirname(url2.fileURLToPath(new URL(import.meta.url)));
  let appWebRoot = normalizePath10(path18.resolve(__dirname, "../app"));
  if (true) {
    appWebRoot = path18.join(__dirname, "app");
  }
...
`;
  ...
  if (storybook2) {
    const packageRoot = path18.dirname(url2.fileURLToPath(import.meta.url));
    const configPath = path18.resolve(packageRoot, "./storybook-main");
    sendTrackingRequest("start-storybook");
    let storybookBin;
    try {
      const sbPackagePath = await import.meta.resolve("storybook/package.json");
      const sbPackageDir = path18.dirname(url2.fileURLToPath(sbPackagePath));
      const packageJson = JSON.parse(
        fs9.readFileSync(path18.join(sbPackageDir, "package.json"), "utf-8")
      );
      storybookBin = path18.join(
        sbPackageDir,
    ...
    const storybookProcess = spawn2(
      ...
      ],
      {
        ...
        env: {
          ...
          hsEnv: authedEnv,
          hublet: authedHublet,
          WEB_ROOT: path18.join(webRoot2, "components")
        },
        stdio: "inherit"
      }
    );
